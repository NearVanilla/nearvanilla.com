...
var highscore = (function () {
  ...
  function calculateDeathsPerDay() {
    const playerPlayedMinutes = generatePlayerScoreMap(
      highscoreObject.scores["ts_PlayTime"].scores
    );
    const playerDeaths = generatePlayerScoreMap(
      highscoreObject.scores["ts_Deaths"].scores
    );

    let deathPerDayScores = [];
    for (let player of playerList) {
      const hoursPlayedStr = playerPlayedMinutes[player];
      if (hoursPlayedStr === undefined) {
        continue;
      }
    ...
    deathPerDayScores = deathPerDayScores
  ...
  function makeHighscoreHeader(highscorename) {
    $("#row").append(
      $("<div>")
        .addClass("col-md-4 p-2")
        .append(
          $("<div>")
            .addClass("hiheader")
            .css("filter", "hue-rotate(" + hue + "deg)")
            .append($("<h1>").append(highscorename))
        )
        .append(
          $("<table>")
            .addClass("highscores")
  ...
  function makePlayerScoreRow(rank, playername, score) {
    ...
    const uuid = uuidByPlayerName[playername]
    const url = uuid ? `https://crafatar.com/avatars/${uuid}?overlay=true&default=MHF_Steve` : "https://crafatar.com/avatars/cd0e7551-facd-1111-a6aa-d09a6a03e222";
    
    const avatarCell =
      typeof playername === "undefined"
        ? ""
        : $("<img>")
            .attr("alt", "Avatar of " + playername)
            .attr("src", url);
    const playerCell =
      typeof playername === "undefined"
        ? "-"
        : $("<a href='#'>").addClass("playername").text(playername);

    return $("<tr>")
      .append($("<td>").addClass("rank").text(rank))
      .append($("<td>").addClass("avatar").append(avatarCell))
      .append($("<td>").append(playerCell))
      ...
      if (nb === "scores") {
        $.each(obj, function (nb, obj) {
          ...
          $.each(obj.scores, function (nb, obj) {
            if (
              obj.playerName.toLowerCase() === selectedUserName.toLowerCase()
            ) {
  ...
  function calculateTimeAndToHtml() {
    $("#highscoreUpdateTimestamp").text(
      timeConverter(highscoreObject["timestamp"])
    );
  }
