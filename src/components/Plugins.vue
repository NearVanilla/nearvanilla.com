<template>
  <div>
    <div
      class="flex w-full md:w-3/4 lg:w-1/2 mb-10 mx-auto justify-between montserrat"
    >
      <p
    ...
    >
      <div v-for="plugin in pluginsFiltered" :key="plugin.name" class="card">
        <div class="card-inner montserrat">
          <div
            class="front flex flex-col justify-between items-center p-6 bg-gray-100 rounded-md basis-0"
          >
          ...
          <div
            class="back bg-gray-100 rounded-md flex flex-col justify-between p-8 items-center h-full"
          >
...
const plugins = ref(pluginsList);

const pluginsFiltered = computed(() =>
  plugins.value.filter((x) => x.type == filter.value || filter.value == "all")
);
